...
const VSHADER_SOURCE = `
  ...
  void main() {
    gl_Position = rotationMatrix * uModelMatrix * vec4(aPosition, 0.0, 1.0);
  }
  `;
...
function connectVariablesToGLSL(){
  ...
  a_Position = gl.getAttribLocation(gl.program, "aPosition");
  if (a_Position < 0) {
  ...
  u_ModelMatrix = gl.getUniformLocation(gl.program, "uModelMatrix");
  if (!u_ModelMatrix) {
    console.log("Failed to get the storage location of uModelMatrix");
    return;
  }

  u_RotationMatrix = gl.getUniformLocation(gl.program, "rotationMatrix");
  if (!u_RotationMatrix) {
    console.log("Failed to get the storage location of rotationMatrix");
    return;
  }
}
